  ...
  data.forEach((element) => {
    const html = element.products
      .map(
        ...
        <h5>${item.price}</h5>
        <a href="#" onClick="add(this)" data-item='${JSON.stringify(
          item,
        )}' class="btn btn-dark">Add to car</a>
        </div>
        </div>
        </div>
        `,
  ...
  if (carrito.length == 0) {
    ...
    html =
      ...
      carrito
        ...
        <td>
        <button class="btn btn-secondary" onclick="mdq(${
          item.item - 1
        },1)">+</button>
        <button class="btn btn-secondary" onclick="mdq(${
          item.item - 1
      ...
      `
        ...
        <button class="btn confirm" onclick="confirm()">Confirm order</button>
        </div>
        </div>
        `;
  }
...
const totalCar = () => {
  let n = 0;
  carrito.forEach((element) => {
    n += element.quantity * element.unitPrice;
  });
  return n;
};
const add = (button) => {
  ...
  if (!e) {
    ...
    item = (({ item, quantity, description, unitPrice }) => ({
      item,
      quantity,
      description,
      unitPrice,
